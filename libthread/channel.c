...
static void addarray(_Altarray *a, Alt *alt) {
	if (a->n == a->m) {
		a->m += 16;
...
static void delarray(_Altarray *a, int i) {
	--a->n;
	a->a[i] = a->a[a->n];
}
...
int chansend(Channel *c, void *v) { return _chanop(c, CHANSND, v, 1); }

int channbsend(Channel *c, void *v) { return _chanop(c, CHANSND, v, 0); }

int chanrecv(Channel *c, void *v) { return _chanop(c, CHANRCV, v, 1); }

int channbrecv(Channel *c, void *v) { return _chanop(c, CHANRCV, v, 0); }
...
int channbsendul(Channel *c, ulong val) { return _chanop(c, CHANSND, &val, 0); }
