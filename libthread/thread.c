...
static _Thread *getthreadnow(void) { return proc()->thread; }

_Thread *(*threadnow)(void) = getthreadnow;

static Proc *procalloc(void) {
	Proc *p;
...
void _threadpthreadmain(Proc *p, _Thread *t) {
	_threadsetproc(p);
	lock(&p->lock);
...
void **procdata(void) { return &proc()->udata; }

void **threaddata(void) { return &proc()->thread->udata; }

extern Jmp *(*_notejmpbuf)(void);

static Jmp *threadnotejmp(void) { return &proc()->sigjmp; }
...
char *threadgetname(void) { return proc()->thread->name; }

void threadsetstate(char *fmt, ...) {
	va_list  arg;
	_Thread *t;
...
static int singlethreaded(void) {
...
static void threadqunlock(QLock *l, ulong pc) {
	_Thread *ready;
 ...
 * sleep and wakeup
 */
static void threadrsleep(Rendez *r, ulong pc) {
	if (singlethreaded()) {
...
static int threadrwakeup(Rendez *r, int all, ulong pc) {
	int      i;
	_Thread *t;
...
int           mainstacksize;
extern int _p9usepwlibrary; /* getgrgid etc. smash the stack - tell _p9dir just
			       say no */
 ...
 */
static void addthread(_Threadlist *l, _Thread *t) {
	if (l->tail) {
		l->tail->next = t;
		t->prev = l->tail;
	} else {
...
static int onlist(_Threadlist *l, _Thread *t) {
	_Thread *tt;
...
static void addthreadinproc(Proc *p, _Thread *t) {
	_Threadlist *l;
 ...
 */
void threadnotify(int (*f)(void *, char *), int in) { atnotify(f, in); }

static int onrunqueue(Proc *p, _Thread *t) {
	_Thread *tt;
 ...
 */
static int threadinfo(void *v, char *s) {
	Proc    *p;
	_Thread *t;
