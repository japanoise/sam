...
int _threadlock(Lock *lk, int block, ulong pc) {
	int r;
...
void _threadunlock(Lock *lk, ulong pc) {
...
void _procwakeup(_Procrendez *r) {
	if (r->asleep) {
		r->asleep = 0;
...
void _procwakeupandunlock(_Procrendez *r) {
	if (r->asleep) {
		r->asleep = 0;
		pthread_cond_signal(&r->cond);
	}
...
static void startpthreadfn(void *v) {
	void   **a;
	Proc    *p;
	_Thread *t;
...
void _procstart(Proc *p, void (*fn)(Proc *)) {
	void **a;
...
void _threadpthreadstart(Proc *p, _Thread *t) {
	void **a;
...
void _threadsetproc(Proc *p) { pthread_setspecific(prockey, p); }

void _pthreadinit(void) {
