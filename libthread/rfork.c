...
int p9rfork(int flags) {
	...
	if ((flags & (RFPROC | RFFDG | RFMEM)) == (RFPROC | RFFDG)) {
		...
		if (flags & RFNOWAIT) {
			...
			if (pid) {
				/*
				 * Parent - wait for child to fork wait-free
				 * child. Then read pid from pipe.  Assume pipe
				 * buffer can absorb the write.
				 */
				close(p[1]);
				status = 0;
				...
				close(p[0]);
				if (!WIFEXITED(status) ||
				    WEXITSTATUS(status) != 0 || n <= 0) {
					if (!WIFEXITED(status)) {
						werrstr("pipe dance - !exited "
							"0x%ux",
							status);
					} else if (WEXITSTATUS(status) != 0) {
						werrstr("pipe dance - non-zero "
							"status 0x%ux",
							status);
					} else if (n < 0) {
						werrstr("pipe dance - pipe "
							"read error - %r");
					} else if (n == 0) {
						werrstr("pipe dance - pipe "
							"read eof");
					} else {
						werrstr("pipe dance - unknown "
							"failure");
					}
					return -1;
				}
				...
				/*
				 * Child - fork a new child whose wait message
				 * can't get back to the parent because we're
				 * going to exit!
				 */
				...
				pid = fork();
				if (pid) {
					/* Child parent - send status over pipe
					 * and exit. */
					if (pid > 0) {
						fprint(p[1], "%d", pid);
					} else {
