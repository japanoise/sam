...
int frdelete(Frame *f, uint64_t p0, uint64_t p1) {
	 ...
	 *  n1, b are box containing beginning of stuff to be kept after
	 * deletion region between pt0 and pt1 is clear
	 */
	b = &f->box[n1];
	...
	if (pt1.y != pt0.y) {
		...
		if (n1 < f->nbox) {
			...
			q2 = pt2.y + f->fheight;
			bitblt2(f->b, pt0, f->b,
				Rect(pt1.x, pt1.y, f->r.max.x, q1), S, 0,
				f->bg);
			bitblt2(f->b, Pt(f->r.min.x, q0), f->b,
				Rect(f->r.min.x, q1, f->r.max.x, q2), S, 0,
				f->bg);
