...
Point _frptofcharptb(Frame *f, uint64_t p, Point pt, int bn) {
	uint8_t *s;
	Frbox   *b;
	int      w, l;
	Rune     r;
...
Point frptofchar(Frame *f, uint64_t p) {
	return _frptofcharptb(f, p, Pt(f->left, f->r.min.y), 0);
}

Point _frptofcharnb(Frame *f, uint64_t p,
...
uint64_t frcharofpt(Frame *f, Point pt) {
	Point    qt;
	int      w, bn, cstart;
	uint8_t *s;
	Frbox   *b;
