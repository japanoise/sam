...
static void Keyaction(Widget w, XEvent *e, String *p, Cardinal *np) {
	...
	for (Keymapping *m = keymappings; m; m = m->next) {
		...
		/* Note that magic bit manipulation here - we want to check that
		 * the modifiers that are specified for the binding are all
		 * pressed, but we allow other modifiers to be as well. This is
		 * because when NumLock is on, it's always added to the modifier
		 * mask.
		 */
...
static Boolean SendSel(Widget w, Atom *sel, Atom *target, Atom *rtype,
...
static String SelectSwap(Widget w, String s) {
	...
	XtGetSelectionValue(w, XInternAtom(_dpy, clipatom, 0),
	...
	XtOwnSelection(w, XInternAtom(_dpy, clipatom, 0),
		       XtLastTimestampProcessed(XtDisplay(w)), SendSel, NULL,
		       NULL);
