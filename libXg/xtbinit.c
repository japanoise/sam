...
void xtbinit(Errfunc f, char *class, int *pargc, char **argv,
	...
	n++;
	widg =
	    XtCreateManagedWidget("gwin", gwinWidgetClass, _toplevel, args, n);
	...
	char bgspec[1024] = {0};
	strncpy(bgspec,
		backgroundspec[0]      ? backgroundspec
		: getenv("BACKGROUND") ? getenv("BACKGROUND")
				       : DEFAULT_BACKGROUND,
		sizeof(bgspec) - 1);

	char *bgc = NULL;
	for (bgc = strtok(bgspec, ":"); bgc != NULL && _nbgs < MAX_BACKGROUNDS;
	     bgc = strtok(NULL, ":")) {
		XColor xc = {0};
	...
	pid_t pid = getpid();
	XChangeProperty(_dpy, XtWindow(_toplevel),
			XInternAtom(_dpy, "_NET_WM_PID", False), XA_CARDINAL,
	...
	xrcolor.alpha = 65535;
	XftColorAllocValue(_dpy, DefaultVisual(_dpy, DefaultScreen(_dpy)),
			   DefaultColormap(_dpy, DefaultScreen(_dpy)), &xrcolor,
			   &fontcolor);
...
static void gotinput(XtPointer cldata, int *pfd, XtInputId *id) {
...
void wrcolmap(Bitmap *b, RGB *map) {
	...
	if (n > 256) {
		...
		if (!XMatchVisualInfo(_dpy, XScreenNumberOfScreen(scr), depth,
				      PseudoColor, &vi)) {
...
uint64_t event(Event *e) { return eread(~0L, e); }

uint64_t eread(uint64_t keys, Event *e) {
	Ebuf *eb;
	int   i;
...
static inline void ebappend(Ebuf *b, Esrc *s) {
	if (s->tail) {
		s->tail->next = b;
		s->tail = b;
	} else {
...
static Ebuf *ebadd(Esrc *s, bool prepend) {
	Ebuf *eb;
	int   m;
...
printgc(char *msg, GC g) {
	...
	XGetGCValues(_dpy, g,
			 ...
			 GCTile | GCFillStyle | GCStipple,
		     &v);
	fprintf(stderr, "%s: gc %x\n", msg, g);
	fprintf(
	    stderr,
	    "  fg %d bg %d func %d fillstyle %d font %x tile %x stipple %x\n",
	    v.foreground, v.background, v.function, v.fill_style, v.font,
	    v.tile, v.stipple);
}
#endif
