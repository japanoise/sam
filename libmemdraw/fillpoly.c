...
static void xscan(Memimage *dst, Seg **seg, Seg *segtab, int nseg, int wind,
		  Memimage *src, Point sp, int, int, int, int);
static void yscan(Memimage *dst, Seg **seg, Seg *segtab, int nseg, int wind,
		  Memimage *src, Point sp, int, int);
...
static void fillline(Memimage *dst, int left, int right, int y, Memimage *src,
		     Point p, int op) {
	Rectangle r;
...
static void fillpoint(Memimage *dst, int x, int y, Memimage *src, Point p,
		      int op) {
	Rectangle r;
...
void memfillpoly(Memimage *dst, Point *vert, int nvert, int w, Memimage *src,
		 Point sp, int op) {
	_memfillpolysc(dst, vert, nvert, w, src, sp, 0, 0, 0, op);
}

void _memfillpolysc(Memimage *dst, Point *vert, int nvert, int w, Memimage *src,
		    Point sp, int detail, int fixshift, int clipped, int op) {
	Seg **seg, *segtab;
	Point p0;
	int   i;
...
static long smuldivmod(long x, long y, long z, long *mod) {
	vlong vx;
...
static void xscan(Memimage *dst, Seg **seg, Seg *segtab, int nseg, int wind,
		  Memimage *src, Point sp, int detail, int fixshift,
		  int clipped, int op) {
	 ...
	 *
		if(clipped && (src->flags&Frepl) && src->depth==8 &&
	 Dx(src->r)==1 && Dy(src->r)==1) { fill = fillcolor; sp.x =
	 membyteval(src);
		}
	 *
	 */
...
static void yscan(Memimage *dst, Seg **seg, Seg *segtab, int nseg, int wind,
		  Memimage *src, Point sp, int fixshift, int op) {
...
static int ycompare(const void *a, const void *b) {
	Seg **s0, **s1;
	long  y0, y1;
...
static int xcompare(const void *a, const void *b) {
	Seg **s0, **s1;
	long  x0, x1;
