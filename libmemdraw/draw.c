 ...
 */
#define CALC11(a, v, tmp) (tmp = (a) * (v) + 128, (tmp + (tmp >> 8)) >> 8)
...
#define CALC21(a, vvuu, tmp)                                                   \
	(tmp = (a) * (vvuu) + 0x00800080,                                      \
	 ((tmp + ((tmp >> 8) & MASK)) >> 8) & MASK)
...
void _memimagedraw(Memdrawparam *par) {
	/*
	 * Now that we've clipped the parameters down to be consistent, we
	 * simply try sub-drawing routines in order until we find one that was
	 * able to handle us.  If the sub-drawing routine returns zero, it means
	 * it was unable to satisfy the request, so we do not return.
	 */
	 ...
	 * res image.
	 */
	DBG printf("test chardraw\n");
	if (chardraw(par)) {
		/*if(drawdebug) iprint("chardraw handled\n"); */
		DBG printf("chardraw handled\n");
		return;
	}
	 ...
	 */
	DBG printf("do alphadraw\n");
	alphadraw(par);
	/*if(drawdebug) iprint("alphadraw handled\n"); */
	DBG printf("alphadraw handled\n");
}
 ...
 */
int drawclip(Memimage *dst, Rectangle *r, Memimage *src, Point *p0,
	     Memimage *mask, Point *p1, Rectangle *sr, Rectangle *mr) {
	Point     rmin, delta;
	int       splitcoords;
	Rectangle omr;
 ...
 * Conversion tables.
 */
static uchar replbit[1 + 8][256]; /* replbit[x][y] is the replication of the
...
static void getparam(Param *p, Memimage *img, Rectangle r, int convgrey,
		     int needbuf) {
	int nbuf;
...
static void clipy(Memimage *img, int *y) {
	int dy;
...
static void dumpbuf(char *s, Buffer b, int n) {
	int    i;
	uchar *p;
 ...
 * is not needed and the channels were already byte-aligned (grey8, rgb24,
 * rgba32, rgb32), the readers need not copy the data: they can simply return
 * pointers to the data. If the destination image is grey and the source is not,
 * it is converted using the NTSC formula.
 *
 * Once we have all the channels, we call either rgbcalc or greycalc, depending
 * on whether the destination image is color.  This is allowed to overwrite the
 * dst buffer (perhaps the actual data, perhaps a copy) with its result.  It
 * should only overwrite the dst buffer with the same format (i.e. red bytes
 * with red bytes, etc.)  A new buffer is returned from the calculator, and that
 * buffer is passed to a function to write it to the destination. If the buffer
 * is already pointing at the destination, the writing function is a no-op.
 */
#define DBG if (drawdebug)

static int alphadraw(Memdrawparam *par) {
	 ...
	 *us to. This requires a boolean (1-bit grey) mask and lack of a source
	 *alpha channel.
	 *
	 * The computation is accomplished by assigning the function pointers as
	 *follows: rdsrc - read and convert source into dst format in a buffer
	 ...
	 *names say, but it avoids a fair amount of code duplication to make
	 *this a case here rather than have a separate booldraw.
	 */
	...
	if (!(src->flags & Falpha) && mask->chan == GREY1 && dst->depth >= 8 &&
		...
		if (mask->flags & Falpha) {
			 ...
			 * have to deal with bit alignment, and I haven't
			 * written that.
			 *
			 * This is a common case for things like ellipse
			 * drawing. When there's no alpha involved and the mask
			 * is boolean, we can avoid all the division and
			 * multiplication.
			 */
	...
	if (drawdebug) {
		printf(
		    "alphadraw: rdsrc=%p rdmask=%p rddst=%p calc=%p wrdst=%p\n",
 ...
 * on the pixels?
 */
static int chanmatch(Buffer *bdst, Buffer *bsrc) {
	uchar *drgb, *srgb;
...
static Buffer alphacalc14(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			  int grey, int op) {
	Buffer obdst;
	int    fd, sadelta;
	int    i, sa, ma, q;
...
static Buffer alphacalc2810(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			    int grey, int op) {
	Buffer obdst;
	int    fs, sadelta;
	int    i, ma, da, q;
...
static Buffer alphacalc3679(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			    int grey, int op) {
	Buffer obdst;
	int    fs, fd, sadelta;
	int    i, sa, ma, da, q;
	...
	for (i = 0; i < dx; i++) {
		...
		if (grey) {
			*bdst.grey = CALC12(fs, *bsrc.grey, fd, *bdst.grey, t);
			...
			if (q) {
				*bdst.rgba = CALC42(fs, *bsrc.rgba, fd,
						    *bdst.rgba, t, t1);
				bsrc.rgba++;
				bdst.rgba++;
				bsrc.alpha += sadelta;
...
static Buffer alphacalc11(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			  int grey, int op) {
	Buffer obdst;
	int    fd, sadelta;
	int    i, sa, ma, q;
	...
	for (i = 0; i < dx; i++) {
		...
		if (grey) {
			*bdst.grey = CALC12(ma, *bsrc.grey, fd, *bdst.grey, t);
			...
			if (q) {
				*bdst.rgba = CALC42(ma, *bsrc.rgba, fd,
						    *bdst.rgba, t, t1);
				bsrc.rgba++;
				bdst.rgba++;
				bsrc.alpha += sadelta;
...
/* source alpha 1 */
static Buffer alphacalcS(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			 int grey, int op) {
	Buffer obdst;
	int    fd;
	int    i, ma;
...
static Buffer boolcalc14(Buffer bdst, Buffer b1, Buffer bmask, int dx, int grey,
			 int op) {
	Buffer obdst;
	int    i, ma, zero;
...
static Buffer boolcalc236789(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			     int grey, int op) {
	Buffer obdst;
	int    fs, fd;
	int    i, ma, da, zero;
	...
	for (i = 0; i < dx; i++) {
		...
		if (grey) {
			if (ma) {
				*bdst.grey =
				    CALC12(fs, *bsrc.grey, fd, *bdst.grey, t);
			} else if (zero) {
				*bdst.grey = 0;
			}
			bsrc.grey += bsrc.delta;
			...
			if (ma) {
				*bdst.red =
				    CALC12(fs, *bsrc.red, fd, *bdst.red, t);
				*bdst.grn =
				    CALC12(fs, *bsrc.grn, fd, *bdst.grn, t);
				*bdst.blu =
				    CALC12(fs, *bsrc.blu, fd, *bdst.blu, t);
			} else if (zero) {
				*bdst.red = *bdst.grn = *bdst.blu = 0;
			}
			bsrc.red += bsrc.delta;
...
static Buffer boolcalc1011(Buffer bdst, Buffer bsrc, Buffer bmask, int dx,
			   int grey, int op) {
	Buffer obdst;
	int    i, ma, zero;
 ...
 */
static Buffer greymaskread(Param *p, uchar *buf, int y) {
	Buffer b;
...
static Buffer readnbit(Param *p, uchar *buf, int y) {
	Buffer    b;
	Memimage *img;
	...
	DBG printf("readnbit dx %d %p=%p+%d*%d, *r=%d fetch %d ", dx, r,
		   p->bytermin, y, p->bwidth, *r, n);
	bits = *r++;
	nbits = 8;
	if ((i = x & (npack - 1))) {
		DBG printf("throwaway %d...", i);
		bits <<= depth * i;
		nbits -= depth * i;
	}
	for (i = 0; i < n; i++) {
	...
	for (i = 0; i < n; i++) {
		...
		*w++ = repl[bits >> sh];
		...
		bits <<= depth;
		nbits -= depth;
		DBG printf("bits %x nbits %d...", bits, nbits);
	}
	dx -= n;
	if (dx == 0) {
		return b;
...
static void writenbit(Param *p, uchar *w, Buffer src) {
	uchar *r;
	u32int bits;
...
static Buffer readcmap(Param *p, uchar *buf, int y) {
	Buffer b;
...
static void writecmap(Param *p, uchar *w, Buffer src) {
...
static Buffer readbyte(Param *p, uchar *buf, int y) {
	Buffer    b;
	Memimage *img;
...
static void writebyte(Param *p, uchar *w, Buffer src) {
	Memimage *img;
	...
	for (i = 0; i < dx; i++) {
		...
		DBG printf("u %.8ux...", u);
		u &= mask;
		DBG printf("&mask %.8ux...", u);
		if (isgrey) {
			u |= ((*grey >> (8 - img->nbits[CGrey])) &
			      img->mask[CGrey])
			     << img->shift[CGrey];
			DBG printf("|grey %.8ux...", u);
			grey += delta;
		} else {
			u |=
...
static Readfn *readfn(Memimage *img) {
	if (img->depth < 8) {
		return readnbit;
	}
	if (img->nbits[CMap] == 8) {
...
static Buffer boolcopy8(Buffer bdst, Buffer bsrc, Buffer bmask, int dx, int i,
			int o) {
	uchar *m, *r, *w, *ew;
...
static Buffer boolcopy16(Buffer bdst, Buffer bsrc, Buffer bmask, int dx, int i,
			 int o) {
	uchar  *m;
	ushort *r, *w, *ew;
...
static Buffer boolcopy24(Buffer bdst, Buffer bsrc, Buffer bmask, int dx, int i,
			 int o) {
	uchar *m;
	uchar *r, *w, *ew;
...
static Buffer boolcopy32(Buffer bdst, Buffer bsrc, Buffer bmask, int dx, int i,
			 int o) {
	uchar  *m;
	u32int *r, *w, *ew;
...
static Buffer genconv(Param *p, uchar *buf, int y) {
	Buffer b;
	int    nb;
	uchar *r, *w, *ew;
...
static Readfn *convfn(Memimage *dst, Param *dpar, Memimage *src, Param *spar) {
 ...
 * when necessary.
 */
u32int _pixelbits(Memimage *i, Point pt) {
	uchar *p;
	u32int val;
	int    off, bpp, npack;
...
static Calcfn *boolcopyfn(Memimage *img, Memimage *mask) {
 ...
 */
static void memsets(void *vp, ushort val, int n) {
	ushort *p, *ep;
...
static void memsetl(void *vp, u32int val, int n) {
	u32int *p, *ep;
...
static void memset24(void *vp, u32int val, int n) {
	uchar *p, *ep;
	uchar  a, b, c;
...
u32int _imgtorgba(Memimage *img, u32int val) {
	uchar  r, g, b, a;
	int    nb, ov, v;
	u32int chan;
	uchar *p;
...
u32int _rgbatoimg(Memimage *img, u32int rgba) {
	u32int chan;
	int    d, nb;
	u32int v;
	uchar *p, r, g, b, a, m;
...
static int memoptdraw(Memdrawparam *par) {
	int       m, y, dy, dx, op;
	u32int    v;
	u16int    u16;
	Memimage *src;
	...
	DBG printf("state %ux mval %ux dd %d\n", par->state, par->mval,
		   dst->depth);
	/*
	...
	if ((par->state & m) == m && (par->srgba & 0xFF) == 0xFF &&
		...
		DBG printf("memopt, dst %p, dst->data->bdata %p\n", dst,
			   dst->data->bdata);
		dwid = dst->width * sizeof(u32int);
		dp = byteaddr(dst, par->r.min);
		v = par->sdval;
		DBG printf("sdval %ud, depth %d\n", v, dst->depth);
		switch (dst->depth) {
		case 1:
		case 2:
		case 4:
			...
			dx -= (ppb - np);
			nb = 8 - np * dst->depth; /* no. bits used on right side
						     of word */
			lm = (1 << nb) - 1;
			DBG printf("np %d x %d nb %d lm %ux ppb %d m %ux\n", np,
			...
			nb = 8 - np * dst->depth; /* no. bits unused on right
						     side of word */
			rm = ~((1 << nb) - 1);
			DBG printf("np %d x %d nb %d rm %ux ppb %d m %ux\n", np,
				   par->r.max.x, nb, rm, ppb, m);

			DBG printf("dx %d Dx %d\n", dx, Dx(par->r));
		...
		case 16:
			...
			memmove(&u16, p, 2);
			v = u16;
			DBG printf("dp=%p; dx=%d; for(y=0; y<%d; y++, "
				   "dp+=%d)\nmemsets(dp, v, dx);\n",
				   dp, dx, dy, dwid);
			for (y = 0; y < dy; y++, dp += dwid) {
				memsets(dp, v, dx);
...
static int chardraw(Memdrawparam *par) {
	u32int    bits;
	...
	if ((par->state & (Replsrc | Simplesrc | Fullsrc | Replmask)) !=
		(Replsrc | Simplesrc | Fullsrc) ||
	...
	for (y = 0; y < dy; y++, rp += maskwid, wp += dstwid) {
		...
		case 8:
			...
			for (x = bx; x > ex; x--, wc++) {
				...
				DBG printf("bits %ux sh %d...", bits, i);
				if ((bits >> i) & 1) {
					*wc = v;
				}
			}
...
void _memfillcolor(Memimage *i, u32int val) {
	u32int bits;
	int    d, y;
	uchar  p[4];
