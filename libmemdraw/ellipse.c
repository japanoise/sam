...
static State *newstate(State *s, int a, int b) {
	s->x = 0;
	s->a = a;
	s->a2 = (vlong)(a * a);
	s->b2 = (vlong)(b * b);
...
void memellipse(Memimage *dst, Point c, int a, int b, int t, Memimage *src,
		Point sp, int op) {
	State in, out;
	...
	if (b < a && u > b * b || a < b && -u > a * a) {
		/*	if(b<a&&(t<<1)>b*b/a || a<b&&(t<<1)>a*a/b)	# very
		 * thick */
 ...
 * a brushed ellipse
 */
static void bellipse(int y, State *s, Param *p) {
	int t, ox, oy, x, nx;
 ...
 */
static void erect(int x0, int y0, int x1, int y1, Param *p) {
	Rectangle r;
 ...
 */
static void epoint(int x, int y, Param *p) {
	Point     p0;
	Rectangle r;
 ...
 */
static void eline(int x0, int y0, int x1, int y1, Param *p) {
	/*	print("L%d %d,%d %d,%d\n", p->t, x0, y0, x1, y1); */
	if (x1 > x0 + 1) {
