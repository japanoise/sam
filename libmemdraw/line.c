...
enum { Arrow1 = 8, Arrow2 = 10, Arrow3 = 3 };
...
static void horline1(Memimage *dst, Point p0, Point p1, int srcval,
		     Rectangle clipr) {
	...
	for (x = p0.x; x <= maxx; x++) {
		if (easy ||
		    (clipr.min.x <= x && clipr.min.y <= y && y < clipr.max.y)) {
			*d ^= (*d ^ srcval) & m;
		}
		if (e > 0) {
			y += dy;
...
static void verline1(Memimage *dst, Point p0, Point p1, int srcval,
		     Rectangle clipr) {
	...
	for (y = p0.y; y <= maxy; y++) {
		if (easy ||
		    (clipr.min.y <= y && clipr.min.x <= x && x < clipr.max.x)) {
			*d ^= (*d ^ srcval) & m;
		}
		if (e > 0) {
			x += dd;
...
static void horliner(Memimage *dst, Point p0, Point p1, Memimage *src,
		     Point dsrc, Rectangle clipr) {
...
static void verliner(Memimage *dst, Point p0, Point p1, Memimage *src,
		     Point dsrc, Rectangle clipr) {
...
static void horline(Memimage *dst, Point p0, Point p1, Memimage *src,
		    Point dsrc, Rectangle clipr) {
...
static void verline(Memimage *dst, Point p0, Point p1, Memimage *src,
		    Point dsrc, Rectangle clipr) {
...
static Memimage *membrush(int radius) {
...
static void discend(Point p, int radius, Memimage *dst, Memimage *src,
		    Point dsrc, int op) {
	Memimage *disc;
	Rectangle r;
...
static void arrowend(Point tip, Point *pp, int end, int sin, int cos,
		     int radius) {
	int x1, x2, x3;
	...
	if (end == Endarrow) {
		...
		x1 = (end >> 5) &
		     0x1FF; /* distance along line from end of line to tip */
		x2 = (end >> 14) &
		     0x1FF; /* distance along line from barb to tip */
		x3 = (end >> 23) & 0x1FF; /* distance perpendicular from edge of
					     line to barb */
	}

	/* comments follow track of right-facing arrowhead */
	pp->x = tip.x + ((2 * radius + 1) * sin / 2 -
			 x1 * cos); /* upper side of shaft */
	pp->y = tip.y - ((2 * radius + 1) * cos / 2 + x1 * sin);
	pp++;
	pp->x = tip.x + ((2 * radius + 2 * x3 + 1) * sin / 2 -
			 x2 * cos); /* upper barb */
	...
	pp++;
	pp->x = tip.x + (-(2 * radius + 1) * sin / 2 -
			 x1 * cos); /* lower side of shaft */
	pp->y = tip.y + ((2 * radius + 1) * cos / 2 - x1 * sin);
}

void _memimageline(Memimage *dst, Point p0, Point p1, int end0, int end1,
		   int radius, Memimage *src, Point sp, Rectangle clipr,
		   int op) {
	/*
	...
	if ((src->flags & Frepl) == 0 &&
	    rectclip(&clipr, rectsubpt(src->r, d)) == 0) {
		return;
	}
	 ...
	 * what pixels to draw.  This means the layer code can call this
	 * routine, using clipr to define the region being written, and get the
	 * same set of pixels regardless of the dicing.
	 */
	...
	if ((p0.x == p1.x || p0.y == p1.y) && (end0 & 0x1F) == Endsquare &&
	    (end1 & 0x1F) == Endsquare) {
		r.min = p0;
		r.max = p1;
		if (p0.x == p1.x) {
	...
	case Endarrow:
		...
		_memfillpolysc(dst, pts, 5, ~0, src,
			       addpt(pts[0], mulpt(d, ICOSSCALE)), 1, 10, 1,
			       op);
		pp[1] = pp[4];
		pp += 2;
	}
	...
	_memfillpolysc(dst, pts, pp - pts, ~0, src,
		       addpt(pts[0], mulpt(d, ICOSSCALE)), 0, 10, 1, op);
	dst->clipr = oclipr;
	return;
}

void memimageline(Memimage *dst, Point p0, Point p1, int end0, int end1,
		  int radius, Memimage *src, Point sp, int op) {
 ...
 */
static void addbbox(Rectangle *r, Point p) {
	if (r->min.x > p.x) {
		r->min.x = p.x;
	}
	if (r->min.y > p.y) {
...
Rectangle memlinebbox(Point p0, Point p1, int end0, int end1, int radius) {
	Rectangle r, r1;
	int       extra;
