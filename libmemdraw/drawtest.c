...
#define DBG if (0)
#define RGB2K(r, g, b)                                                         \
	((299 * ((u32int)(r)) + 587 * ((u32int)(g)) + 114 * ((u32int)(b))) /   \
	 1000)
 ...
 */
static void Bprintr5g6b5(Biobuf *bio, char *_, u32int v) {
	int r, g, b;
	r = (v >> 11) & 31;
	g = (v >> 5) & 63;
	b = v & 31;
...
static void Bprintr5g5b5a1(Biobuf *bio, char *_, u32int v) {
	int r, g, b, a;
	r = (v >> 11) & 31;
	g = (v >> 6) & 31;
	b = (v >> 1) & 31;
...
void dumpimage(char *name, Memimage *img, void *vdata, Point labelpt) {
	Biobuf b;
	uchar *data;
	uchar *p;
	char  *arg;
	...
	Bprint(&b, "%s\t%d\tr %R clipr %R repl %d data %p *%P\n", name, r.min.x,
	       r, img->clipr, (img->flags & Frepl) ? 1 : 0, vdata, labelpt);
	mask = (1ULL << bpp) - 1;
	...
	for (y = 0; y < Yrange; y++) {
		...
		v = 0;
		p = data +
		    (byteaddr(img, Pt(0, y)) - (uchar *)img->data->bdata);
		Bprint(&b, "%-4d\t", y);
 ...
 */
void checkone(Point p, Point sp, Point mp) {
	int    delta;
	uchar *dp, *sdp;
...
void checkline(Rectangle r, Point sp, Point mp, int y, Memimage *stmp,
	       Memimage *mtmp) {
	u32int *dp;
	int     nb;
	u32int *saved;
 ...
 */
void fill(Memimage *img, uchar *ucbits) {
	int     i, x, y;
	ushort *up;
	uchar   alpha, r, g, b;
	void   *data;
 ...
 */
void verifyonemask(void) {
	Point dp, sp, mp;

	fill(dst, dstbits);
	fill(src, srcbits);
	memmove(dst->data->bdata, dstbits,
		dst->width * sizeof(u32int) * Yrange);
	memmove(src->data->bdata, srcbits,
		src->width * sizeof(u32int) * Yrange);
	memmove(mask->data->bdata, maskbits,
		mask->width * sizeof(u32int) * Yrange);
	...
	memmove(dst->data->bdata, dstbits,
		dst->width * sizeof(u32int) * Yrange);
	memimagedraw(dst, Rect(dp.x, dp.y, dp.x + 1, dp.y + 1), src, sp, mask,
		     mp, SoverD);
	memmove(mask->data->bdata, maskbits,
		mask->width * sizeof(u32int) * Yrange);
...
void verifylinemask(void) {
	...
	tp = sp;
	up = mp;
	for (x = dr.min.x; x < dr.max.x && tp.x < Xrange && up.x < Xrange;
	     x++, tp.x++, up.x++) {
		memimagedraw(dst, Rect(x, dr.min.y, x + 1, dr.min.y + 1), src,
			     tp, mask, up, SoverD);
	}
	memmove(savedstbits, dst->data->bdata,
		dst->width * sizeof(u32int) * Yrange);
...
void verifyrectmask(void) {
	...
	tp = sp;
	up = mp;
	for (y = dr.min.y; y < dr.max.y && tp.y < Yrange && up.y < Yrange;
	     y++, tp.y++, up.y++) {
		for (x = dr.min.x;
		     x < dr.max.x && tp.x < Xrange && up.x < Xrange;
		     x++, tp.x++, up.x++) {
			memimagedraw(dst, Rect(x, y, x + 1, y + 1), src, tp,
				     mask, up, SoverD);
		}
		tp.x = sp.x;
		up.x = mp.x;
 ...
 */
int tilexy(int minx, int maxx, int x) {
	int sx;
...
void replicate(Memimage *i, Memimage *tmp) {
	Rectangle r, r1;
	int       x, y, nb;
 ...
 * Mask is preset; do the rest
 */
void verifyrectmaskrepl(int srcrepl, int maskrepl) {
	Point     sp, mp, tp, up;
	Rectangle dr;
	int       x, y;
	Memimage *s, *m;
	...
	for (tp.y = sp.y, up.y = mp.y, y = dr.min.y;
	     y < dr.max.y && tp.y < Yrange && up.y < Yrange;
	     y++, tp.y++, up.y++) {
		for (tp.x = sp.x, up.x = mp.x, x = dr.min.x;
		     x < dr.max.x && tp.x < Xrange && up.x < Xrange;
		     x++, tp.x++, up.x++) {
			memimagedraw(dst, Rect(x, y, x + 1, y + 1), s, tp, m,
				     up, SoverD);
		}
	}
	memmove(savedstbits, dst->data->bdata,
...
void verifyrectrepl(int srcrepl, int maskrepl) {
	int i;
 ...
 */
u32int replbits(u32int v, int nhave, int nwant) {
	v &= (1 << nhave) - 1;
 ...
 */
void pixtorgba(u32int v, uchar *r, uchar *g, uchar *b, uchar *a) {
	*a = v >> 24;
	*r = v >> 16;
	*g = v >> 8;
	*b = v;
 ...
 */
u32int rgbatopix(uchar r, uchar g, uchar b, uchar a) {
 ...
 */
u32int getpixel(Memimage *img, Point pt) {
	uchar  r, g, b, a, *p;
	int    nbits, npack, bpp;
	...
	if (bpp < 8) {
		 ...
		 *
		 * We want to throw away the top pt.x%npack pixels and then use
		 * the next bpp bits in the bottom byte of v.  This madness is
		 * due to having big endian bits but little endian bytes.
		 */
		npack = 8 / bpp;
		...
		/*
		 * General case.  We need to parse the channel descriptor and do
		 * what it says. In all channels but the color map, we replicate
		 * to 8 bits because that's the precision that all calculations
		 * are done at.
		 *
		 * In the case of the color map, we leave the bits alone, in
		 * case a color map with less than 8 bits of index is used. This
		 * is currently disallowed, so it's sort of silly.
		 */
 ...
 */
uchar getgrey(Memimage *img, Point pt) {
	uchar r, g, b, a;
 ...
 */
uchar getmask(Memimage *img, Point pt) {
	if (img->flags & Falpha) {
...
void putpixel(Memimage *img, Point pt, u32int nv) {
	...
	if (bpp < 8) {
		/*
		 * Sub-byte greyscale pixels.  We need to skip the leftmost
		 * pt.x%npack pixels, which is equivalent to skipping the
		 * rightmost npack - pt.x%npack - 1 pixels.
		 */
		npack = 8 / bpp;
		sh = bpp * (npack - pt.x % npack - 1);
		bits = RGB2K(r, g, b);
		DBG print("repl %lux 8 %d = %lux...", bits, bpp,
			  replbits(bits, 8, bpp));
		...
		mask <<= sh;
		bits <<= sh;
		DBG print("(%lux & %lux) | (%lux & %lux)", v, ~mask, bits,
			  mask);
		v = (v & ~mask) | (bits & mask);
	} else {
		/*
		...
		for (c = img->chan; c; c >>= 8) {
			...
			DBG print("repl %lux 8 %d = %lux...", bits, nbits,
				  replbits(bits, 8, nbits));
			if (TYPE(c) != CMap) {
				bits = replbits(bits, 8, nbits);
			}
			mask = (1 << nbits) - 1;
			DBG print("bits %lux mask %lux sh %d...", bits, mask,
				  sh);
			bits <<= sh;
			mask <<= sh;
			v = (v & ~mask) | (bits & mask);
...
void drawonepixel(Memimage *dst, Point dp, Memimage *src, Point sp,
		  Memimage *mask, Point mp) {
	...
	DBG print("dst %x %x %x %x src %x %x %x %x m %x = ", dr, dg, db, da, sr,
		  sg, sb, sa, m);
	if (dst->flags & Fgrey) {
		/*
		 * We need to do the conversion to grey before the alpha
		 * calculation because the draw operator does this, and we need
		 * to be operating at the same precision so we get exactly the
		 * same answers.
		 */
		sk = RGB2K(sr, sg, sb);
		dk = RGB2K(dr, dg, db);
		 ...
		 * alpha) the same.  It might have been nice to use an array,
		 * but oh well.
		 */
