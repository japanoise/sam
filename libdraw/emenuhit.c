...
static void menucolors(void) {
	/* Main tone is greenish, with negative selection */
	back = allocimagemix(display, DPalegreen, DWhite);
	high = allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1,
			  DDarkgreen); /* dark green */
	bord = allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1,
...
static void paintitem(Menu *menu, Rectangle textr, int off, int i,
		      int highlight, Image *save, Image *restore) {
	char     *item;
	Rectangle r;
	Point     pt;
	...
	r = menurect(textr, i);
	if (restore) {
		draw(screen, r, restore, NULL, restore->r.min);
		return;
	}
	if (save) {
		draw(save, save->r, screen, NULL, r.min);
	}
	item = menu->item ? menu->item[i + off] : (*menu->gen)(i + off);
	pt.x = (textr.min.x + textr.max.x - stringwidth(font, item)) / 2;
	pt.y = textr.min.y + i * (font->height + Vspacing);
	draw(screen, r, highlight ? high : back, NULL, pt);
 ...
 */
static int menuscan(Menu *menu, int but, Mouse *m, Rectangle textr, int off,
		    int lasti, Image *save) {
	int i;

	paintitem(menu, textr, off, lasti, 1, save, NULL);
	...
	*m = emouse();
	while (m->buttons & (1 << (but - 1))) {
		...
		*m = emouse();
		...
		paintitem(menu, textr, off, lasti, 0, NULL, save);
		if (i == -1) {
			return i;
		}
		lasti = i;
...
static void menupaint(Menu *menu, Rectangle textr, int off, int nitemdrawn) {
	int i;

	draw(screen, insetrect(textr, Border - Margin), back, NULL, ZP);
	for (i = 0; i < nitemdrawn; i++) {
		paintitem(menu, textr, off, i, 0, NULL, NULL);
	}
}

static void menuscrollpaint(Rectangle scrollr, int off, int nitem,
			    int nitemdrawn) {
	Rectangle r;

	draw(screen, scrollr, back, NULL, ZP);
	...
	if (menutxt == 0) {
		menutxt =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, DDarkgreen);
	}
	if (menutxt) {
		draw(screen, insetrect(r, 1), menutxt, NULL, ZP);
	}
}
...
int emenuhit(int but, Mouse *m, Menu *menu) {
	...
	r = insetrect(Rect(0, 0, wid, nitemdrawn * (font->height + Vspacing)),
		      -Margin);
	r = rectsubpt(r, Pt(wid / 2, lasti * (font->height + Vspacing) +
					 font->height / 2));
	r = rectaddpt(r, m->xy);
	pt = ZP;
	...
	draw(b, menur, screen, NULL, menur.min);
	draw(screen, menur, back, NULL, ZP);
		...
		while (!ptinrect(m->xy, textr) &&
			...
			if (scrolling && ptinrect(m->xy, scrollr)) {
				...
				if (noff != off) {
					off = noff;
					menupaint(menu, textr, off, nitemdrawn);
					menuscrollpaint(scrollr, off, nitem,
							nitemdrawn);
				}
			}
			flushimage(display,
				   1); /* in case display->locking is set */
			*m = emouse();
		}
	}
	draw(screen, menur, b, NULL, menur.min);
	if (b != screen) {
		freeimage(b);
	}
	freeimage(save);
