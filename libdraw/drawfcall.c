...
static int GETSTRING(uchar *p, char **s) {
	int n;
...
uint sizeW2M(Wsysmsg *m) {
	...
	case Tcursor2:
		return 4 + 1 + 1 + 4 + 4 + 2 * 16 + 2 * 16 + 4 + 4 + 4 * 32 +
		       4 * 32 + 1;
	case Rerror:
	...
	case Tctxt:
		return 4 + 1 + 1 + _stringsize(m->id);
	case Tinit:
		return 4 + 1 + 1 + _stringsize(m->winsize) +
		       _stringsize(m->label);
	case Rrdsnarf:
	case Twrsnarf:
...
uint convW2M(Wsysmsg *m, uchar *p, uint n) {
	int nn;
...
uint convM2W(uchar *p, uint n, Wsysmsg *m) {
	int nn;
...
int drawfcallfmt(Fmt *fmt) {
	...
	case Rrdmouse:
		return fmtprint(
		    fmt, "Rrdmouse x=%d y=%d buttons=%d msec=%d resized=%d",
	...
	case Tmoveto:
		return fmtprint(fmt, "Tmoveto x=%d y=%d", m->mouse.xy.x,
				m->mouse.xy.y);
	case Rmoveto:
	...
	case Tinit:
		return fmtprint(fmt, "Tinit label='%s' winsize='%s'", m->label,
				m->winsize);
	case Rinit:
	...
	case Rrddraw:
		return fmtprint(fmt, "Rrddraw %d %.*H", m->count, m->count,
				m->data);
	case Twrdraw:
		return fmtprint(fmt, "Twrdraw %d %.*H", m->count, m->count,
				m->data);
	case Rwrdraw:
