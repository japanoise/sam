...
static void menucolors(void) {
	...
	high = allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1,
...
static void paintitem(Image *m, Menu *menu, Rectangle textr, int off, int i,
 ...
 */
static int menuscan(Image *m, Menu *menu, int but, Mousectl *mc,
...
static void menupaint(Image *m, Menu *menu, Rectangle textr, int off,
		      int nitemdrawn) {
	int i;
...
static void menuscrollpaint(Image *m, Rectangle scrollr, int off, int nitem,
			    int nitemdrawn) {
	Rectangle r;
	...
	if (menutxt == 0) {
		menutxt = allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1,
...
int menuhit(int but, Mousectl *mc, Menu *menu, Screen *scr) {
	...
	while (mc->m.buttons & (1 << (but - 1))) {
		...
		while (!ptinrect(mc->m.xy, textr) &&
		       (mc->m.buttons & (1 << (but - 1)))) {
			if (scrolling && ptinrect(mc->m.xy, scrollr)) {
				noff = ((mc->m.xy.y - scrollr.min.y) * nitem) /
				       Dy(scrollr);
				noff -= nitemdrawn / 2;
				if (noff < 0) {
					noff = 0;
