...
Subfont *_getsubfont(Display *d, char *name) {
	int      fd;
	Subfont *f;
	int      scale;
	char    *fname;
...
static void scalesubfont(Subfont *f, int scale) {
	Image    *i;
	Rectangle r, r2;
	int       y, x, x2, j;
	uchar    *src, *dst;
	...
	for (y = r.min.y; y < r.max.y; y++) {
		...
		for (x = 0; x < Dx(r); x++) {
			v = ((src[x / pack] << ((x % pack) * f->bits->depth)) >>
			     (8 - f->bits->depth)) &
			    mask;
			for (j = 0; j < scale; j++) {
				x2 = x * scale + j;
		...
		for (j = 0; j < scale; j++) {
			loadimage(i,
				  Rect(r2.min.x, y * scale + j, r2.max.x,
				       y * scale + j + 1),
				  dst, dstn);
		}
	}
