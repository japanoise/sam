...
int cachechars(Font *f, char **ss, Rune **rr, ushort *cp, int max, int *wp,
	       char **subfontname) {
	...
	*subfontname = 0;
	for (i = 0; i < max && (*sp || *rp); sp += w, rp += rw) {
		...
		if (a &&
		    (f->age - a) < 500) { /* kicking out too recent; resize */
...
static Subfont *cf2subfont(Cachefont *cf, Font *f) {
	int      depth;
	char    *name;
	Subfont *sf;
...
int loadchar(Font *f, Rune r, Cacheinfo *c, int h, int noflush,
	     char **subfontname) {
...
static int fontresize(Font *f, int wid, int ncache, int depth) {
	Cacheinfo *i;
	int        ret;
	Image *new;
	uchar   *b;
