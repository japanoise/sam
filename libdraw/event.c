...
ulong event(Event *e) { return eread(~0UL, e); }
...
static Ebuf *newebuf(Slave *s, int n) {
	Ebuf *eb;
...
static Muxrpc *startrpc(int type) {
	uchar   buf[512];
	Wsysmsg w;
...
static int finishrpc(Muxrpc *r, Wsysmsg *w) {
	uchar *p;
	void  *v;
	int    n;
...
static int extract(int canblock) {
	...
	} else if (display->bufp > display->buf) {
		flushimage(display, 1);
	}
	...
	for (i = 0; i < nslave; i++) {
		...
		if (i == Smouse) {
			...
			if (t0 >= eslave[i].nexttick) {
				...
				tv.tv_sec =
				    (eslave[i].nexttick - t0) / 1000000000;
				tv.tv_usec = (eslave[i].nexttick - t0) %
					     1000000000 / 1000;
			}
			timeout = &tv;
		} else {
	...
	for (i = 0; i < nslave; i++) {
		...
		if (i == Smouse) {
			...
			if (FD_ISSET(eslave[i].fd, &rset)) {
				...
				eb->n =
				    read(eslave[i].fd, eb->u.buf, eslave[i].n);
				n++;
			}
		}
	}
