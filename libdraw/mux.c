...
static Muxrpc *allocmuxrpc(Mux *mux) {
	Muxrpc *r;

	/* must malloc because stack could be private */
	r = calloc(sizeof(Muxrpc), 1);
	if (r == NULL) {
		werrstr("mallocz: %r");
		return NULL;
	}
...
static int tagmuxrpc(Muxrpc *r, void *tx) {
	int  tag;
	Mux *mux;
...
void muxmsgandqlock(Mux *mux, void *p) {
	int     tag;
	Muxrpc *r2;
...
void *muxrpc(Mux *mux, void *tx) {
	int     tag;
	Muxrpc *r;
	void   *p;

	if ((r = allocmuxrpc(mux)) == NULL) {
		return NULL;
	}
...
Muxrpc *muxrpcstart(Mux *mux, void *tx) {
	int     tag;
	Muxrpc *r;
...
int muxrpccanfinish(Muxrpc *r, void **vp) {
	void *p;
	Mux  *mux;
	int   ret;
...
static void enqueue(Mux *mux, Muxrpc *r) {
...
static void dequeue(Mux *mux, Muxrpc *r) {
	r->next->prev = r->prev;
	r->prev->next = r->next;
	r->prev = NULL;
	r->next = NULL;
...
static int gettag(Mux *mux, Muxrpc *r) {
	int      i, mw;
	Muxrpc **w;
	...
	for (;;) {
		...
		while (mux->nwait == mux->mwait) {
			if (mux->mwait < mux->maxtag - mux->mintag) {
				...
				memset(w + mux->mwait, 0,
				       (mw - mux->mwait) * sizeof(w[0]));
				mux->wait = w;
...
static void puttag(Mux *mux, Muxrpc *r) {
	int i;
