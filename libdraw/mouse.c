...
static void _ioproc(void *arg) {
	...
	for (;;) {
		...
		send(mc->c, &m);
		/*
		 * mc->m is updated after send so it doesn't have wrong value if
		 * we block during send. This means that programs should receive
		 * into mc->Mouse (see readmouse() above) if they want full
		 * synchrony.
		 */
		mc->m = m;
	}
