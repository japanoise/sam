 ...
 */
int memlorigin(Memimage *i, Point log, Point scr) {
	Memlayer  *l;
	Memscreen *s;
	...
	nsave = NULL;
	if (eqlog == 0 && l->save != NULL) {
		nsave = allocmemimage(
		    Rect(log.x, log.y, log.x + Dx(oldr), log.y + Dy(oldr)),
		    i->chan);
		if (nsave == NULL) {
			return -1;
		}
	 ...
	 * previously hidden.
	 */
	shad = memlalloc(s, oldr, memlnorefresh, NULL, DNofill);
	if (shad == NULL) {
		return -1;
	}
	s->frontmost = i;
	 ...
	 */
	for (t = l->rear->layer->rear; t != NULL; t = t->layer->rear) {
		x = newr;
	 ...
	 */
	if (wasclear) {
		memdraw(s->image, newr, s->image, oldr.min, NULL, Pt(0, 0), S);
	} else {
		memlexpose(i, newr);
	}
	memldelete(shad);
