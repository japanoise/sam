...
#define RECUR(a, b, c, d)                                                      \
	_layerop(fn, i, Rect(a.x, b.y, c.x, d.y), clipr, etc,                  \
		 front->layer->rear);

static void _layerop(void (*fn)(Memimage *, Rectangle, Rectangle, void *, int),
		     Memimage *i, Rectangle r, Rectangle clipr, void *etc,
		     Memimage *front) {
	Rectangle fr;
...
Top:
	...
	if (rectXrect(r, fr) == 0) {
		...
		/* assert(front && front->layer && front->layer->screen &&
		 * front->layer->rear); */
...
/*
 * Assumes incoming rectangle has already been clipped to i's logical r and
 * clipr
 */
void _memlayerop(
    void (*fn)(Memimage *, Rectangle, Rectangle, void *, int), Memimage *i,
