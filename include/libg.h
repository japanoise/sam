...
struct Event {
	...
	unsigned char
	    data[EMAXMSG]; /* message from an arbitrary file descriptor */
};
...
enum {
	...
	Kraw, /* insert raw character code, subject to transformation (e.g. tab
		 expansion) */
...
extern void	 bitblt2(Bitmap *, Point, Bitmap *, Rectangle, Fcode, uint64_t,
			 uint64_t);
