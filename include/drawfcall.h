#ifndef _DRAW_F_CALL_H
#define _DRAW_F_CALL_H
...
#define GET(p, x)                                                              \
	((x) = (u32int)(((p)[0] << 24) | ((p)[1] << 16) | ((p)[2] << 8) |      \
			((p)[3])))

#define PUT2(p, x) (p)[0] = ((x) >> 8) & 0xFF, (p)[1] = (x) & 0xFF

#define GET2(p, x) ((x) = (((p)[0] << 8) | ((p)[1])))
