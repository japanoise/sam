...
int main(int argc, char *argv[]) {
	...
	if (getenv("SAMRC")) {
		...
		if (xdg_config_home) {
			snprintf(rcpath, PATH_MAX, "%s/sam/samrc",
				 xdg_config_home);
		} else {
			snprintf(rcpath, PATH_MAX, "%s/.config/sam/samrc",
				 home ? home : ".");
		}
	}
...
int getr(Rectangle *rp) {
	...
	*rp = getrect(3, &mouse);
	if (rp->max.x && rp->max.x - rp->min.x <= 5 &&
	    rp->max.y - rp->min.y <= 5) {
		p = rp->min;
	...
	return rectclip(rp, screen.r) && rp->max.x - rp->min.x > 100 &&
	       rp->max.y - rp->min.y > 40;
}

void snarf(Text *t, int w) {
	Flayer *l = &t->l[w];
...
void cut(Text *t, int w, bool save, bool check) {
	int64_t p0, p1;
	Flayer *l;
...
void scrorigin(Flayer *l, int but, int64_t p0) {
	Text *t = (Text *)l->user1;
...
int64_t ctlw(Rasp *r, int64_t o, int64_t p) {
	int c;
...
int64_t ctlu(Rasp *r, int64_t o, int64_t p) {
...
static int64_t cmdscrollup(Flayer *l, int64_t a, Text *t, const char *arg) {
	flushtyping(false);
	outTslll(Torigin, t->tag, l->origin, l->f.maxlines + 1, getlayer(l, t));
	return a;
}

static int64_t cmdcharleft(Flayer *l, int64_t a, Text *t, const char *arg) {
	flsetselect(l, a, a);
	flushtyping(false);
	if (a > 0) {
		a--;
...
static int64_t cmdcharright(Flayer *l, int64_t a, Text *t, const char *arg) {
	flsetselect(l, a, a);
	flushtyping(false);
	if (a < t->rasp.nrunes) {
		a++;
...
static int64_t cmdeol(Flayer *l, int64_t a, Text *t, const char *arg) {
	flsetselect(l, a, a);
	flushtyping(true);
...
static int64_t cmdscrollupline(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (l->origin > 0) {
...
static int64_t cmdscrolldownline(Flayer *l, int64_t a, Text *t,
	...
	int64_t target =
	    l->origin +
	    frcharofpt(&l->f, Pt(l->f.r.min.x, l->f.r.min.y + l->f.fheight));
...
static int64_t cmdlineup(Flayer *l, int64_t a, Text *t, const char *arg) {
	flsetselect(l, a, a);
	flushtyping(true);
	if (a > 0) {
...
static int64_t cmdlinedown(Flayer *l, int64_t a, Text *t, const char *arg) {
	flsetselect(l, a, a);
	flushtyping(true);
	if (a < t->rasp.nrunes) {
		int64_t p0, count = 0;
...
static int64_t cmdjump(Flayer *l, int64_t a, Text *u, const char *arg) {
	Text *t = NULL;
...
static int64_t cmdlook(Flayer *l, int64_t a, Text *t, const char *arg) {
	outTsll(Tlook, t->tag, which->p0, which->p1);
	setlock();
	return a;
}

static int64_t cmdsearch(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (t != &cmd && haspat()) {
		outcmd();
		outT0(Tsearch);
		setlock();
...
static int64_t cmdwrite(Flayer *l, int64_t a, Text *t, const char *arg) {
...
static int64_t cmdescape(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (typeesc >= 0) {
		l->p0 = typeesc;
		l->p1 = a;
		flushtyping(true);
...
static int64_t cmddelword(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (l->f.p0 > 0 && a > 0) {
...
static int64_t cmddelbol(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (l->f.p0 > 0 && a > 0) {
...
static int64_t cmddelbs(Flayer *l, int64_t a, Text *t, const char *arg) {
	if (l->f.p0 > 0 && a > 0) {
		l->p0 = a - 1;
	}
...
static int64_t cmddel(Flayer *l, int64_t a, Text *t, const char *arg) {
	l->p0 = a;
	if (a < t->rasp.nrunes) {
		l->p1 = a + 1;
	}
...
static int64_t cmdexchange(Flayer *l, int64_t a, Text *t, const char *arg) {
	int w = getlayer(l, t);
	if (w >= 0) {
		snarf(t, w);
		outT0(Tstartsnarf);
...
static int64_t cmdsnarf(Flayer *l, int64_t a, Text *t, const char *arg) {
	flushtyping(false);
...
static int64_t cmdcut(Flayer *l, int64_t a, Text *t, const char *arg) {
	flushtyping(false);
...
static int64_t cmdpaste(Flayer *l, int64_t a, Text *t, const char *arg) {
	flushtyping(false);
...
static int64_t cmdtab(Flayer *l, int64_t a, Text *t, const char *arg) {
	flushtyping(false);
...
static int64_t cmdsend(Flayer *l, int64_t a, Text *t, const char *arg) {
	bool dojump = (t != &cmd);
...
void outcmd(void) {
	if (work) {
		outTsll(Tworkfile, ((Text *)work->user1)->tag, work->p0,
			work->p1);
	}
}
