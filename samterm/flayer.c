...
Rectangle flrect(Flayer *l, Rectangle r) {
	rectclip(&r, lDrect);
	l->entire = r;
	l->scroll = inset(r, FLMARGIN);
	r.min.x = l->scroll.max.x =
	    r.min.x + FLMARGIN + FLSCROLLWID + (FLGAP - FLMARGIN);
	return r;
}

void flinit(Flayer *l, Rectangle r, XftFont *ft, uint64_t bg) {
	lldelete(l);
	llinsert(l);
	l->visible = All;
...
void flclose(Flayer *l) {
	if (l->visible == All) {
		bitblt2(&screen, l->entire.min, &screen, l->entire, 0, _bgpixel,
			_bgpixel);
		...
		if (l->f.b) {
			bitblt2(l->f.b, l->entire.min, l->f.b, l->entire, 0,
				_bgpixel, _bgpixel);
...
void flborder(Flayer *l, bool wide) {
	if (flprepare(l)) {
...
void newvisibilities(int redraw)
...
#define V(a, b) (((a) << 2) | ((b)))
		...
		case V(Some, All):
			if (l->f.b) {
				bitblt2(&screen, l->entire.min, l->f.b,
					l->entire, S, 0, l->bg);
				bfree(l->f.b);
				l->f.b = &screen;
				break;
...
void fldelete(Flayer *l, int64_t p0, int64_t p1) {
	if (flprepare(l)) {
		p0 -= l->origin;
		if (p0 < 0) {
			p0 = 0;
...
bool flselect(Flayer *l) {
	if (l->visible != All) {
		flupfront(l);
	}
	if (mouse.msec - l->click < Clicktime &&
	    (l->f.p0 == l->f.p1 && l->f.p0 == frcharofpt(&l->f, mouse.xy))) {
		l->click = 0;
		return true;
	}
	frselect(&l->f, &mouse);
...
void flsetselect(Flayer *l, int64_t p0, int64_t p1) {
	uint64_t fp0, fp1;
...
void flfp0p1(Flayer *l, uint64_t *pp0, uint64_t *pp1) {
...
Rectangle rscale(Rectangle r, Point old, Point new) {
