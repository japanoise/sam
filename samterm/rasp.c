...
void rresize(Rasp *r, int64_t a, int64_t old, int64_t new) {
	Section *s, *t, *ns;
...
void rdata(Rasp *r, int64_t p0, int64_t p1, wchar_t *cp) {
	Section *s, *t, *ns;
...
wchar_t *rload(Rasp *r, uint64_t p0, uint64_t p1, uint64_t *nrp) {
	Section *s;
	int64_t	 p;
	int	 n, nb;
	...
	while (p < p1 && s) {
		 ...
		 * 'rdata' call, it's because we have an 'rresize' hole here, so
		 * the screen doesn't have data for that space anyway (it got
		 * cut first).  So pretend it isn't there.
		 */
		if (s->text) {
			n = s->nrunes - (p0 - p);
