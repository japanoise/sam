...
void startnewfile(int type, Text *t) {
	t->tag = Untagged;
...
int64_t inlong(int n) {
	...
	l = (indata[n + 7] << 24) | (indata[n + 6] << 16) |
	    (indata[n + 5] << 8) | indata[n + 4];
	l = (l << 16) | (indata[n + 3] << 8) | indata[n + 2];
	l = (l << 16) | (indata[n + 1] << 8) | indata[n];
	return l;
}

void outT0(Tmesg type) {
	outstart(type);
	outsend();
}

void outTl(Tmesg type, int64_t l) {
	outstart(type);
	outlong(l);
	outsend();
}

void outTs(Tmesg type, int s) {
	outstart(type);
	outshort(s);
	outsend();
}

void outTss(Tmesg type, int s1, int s2) {
	outstart(type);
	outshort(s1);
	outshort(s2);
	outsend();
...
void outTslll(Tmesg type, int s1, int64_t l1, int64_t l2, int64_t l3) {
	outstart(type);
	outshort(s1);
	outlong(l1);
	outlong(l2);
...
void outTsll(Tmesg type, int s1, int64_t l1, int64_t l2) {
	outstart(type);
	outshort(s1);
	outlong(l1);
	outlong(l2);
...
void outTsl(Tmesg type, int s1, int64_t l1) {
	outstart(type);
	outshort(s1);
	outlong(l1);
	outsend();
...
void outTsls(Tmesg type, int s1, int64_t l1, int s2) {
	outstart(type);
	outshort(s1);
	outlong(l1);
	outshort(s2);
...
void hmoveto(int m, int64_t p0, Flayer *l) {
	Text *t = whichtext(m);
	l = l ? l : &t->l[t->front];
...
void flnewlyvisible(Flayer *l) { hcheck(((Text *)l->user1)->tag); }

void hsetsnarf(int nc) {
	char *s2;
	char *s1;
	int   i;
	int   n;
