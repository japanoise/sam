...
static void replyerror(Client *c, Wsysmsg *m) {
	char err[256];
 ...
 */
static void runmsg(Client *c, Wsysmsg *m) {
	...
	case Trdmouse:
		qlock(&c->eventlk);
		if ((c->mousetags.wi + 1) % nelem(c->mousetags.t) ==
		    c->mousetags.ri) {
			qunlock(&c->eventlk);
	...
	case Trdkbd4:
		qlock(&c->eventlk);
		if ((c->kbdtags.wi + 1) % nelem(c->kbdtags.t) ==
		    c->kbdtags.ri) {
			qunlock(&c->eventlk);
		...
		c->kbdtags.t[c->kbdtags.wi++] =
		    (m->tag << 1) | (m->type == Trdkbd4);
 ...
 * Reply to m.
 */
static void replymsg(Client *c, Wsysmsg *m) {
	int n;
...
static void matchmouse(Client *c) {
	Wsysmsg m;
	...
	while (c->mouse.ri != c->mouse.wi &&
	       c->mousetags.ri != c->mousetags.wi) {
		m.type = Rrdmouse;
...
void gfx_mouseresized(Client *c) { gfx_mousetrack(c, -1, -1, -1, -1); }

void gfx_mousetrack(Client *c, int x, int y, int b, uint ms) {
	Mouse *m;
...
void gfx_abortcompose(Client *c) {
	qlock(&c->eventlk);
	if (c->kbd.alting) {
		c->kbd.alting = 0;
		c->kbd.nk = 0;
...
// It is called from the graphics thread with no locks held.
void gfx_keystroke(Client *c, int ch) {
	int i;
