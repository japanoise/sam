...
#define Mask                                                                   \
	MouseMask | ExposureMask | StructureNotifyMask | KeyPressMask |        \
...
static ClientImpl x11impl = {rpc_resizeimg,   rpc_resizewindow, rpc_setcursor,
			     rpc_setlabel,    rpc_setmouse,     rpc_topwin,
			     rpc_bouncemouse, rpc_flush};

static Xwin *newxwin(Client *c) {
	Xwin *w;
...
static int xioerror(XDisplay *d) {
	/*print("X I/O error\n"); */
	exit(0);
...
void xlock(void) { qlock(&xlk); }

void xunlock(void) { qunlock(&xlk); }
	...
	*/
	...
	if (XMatchVisualInfo(_x.display, xrootid, 24, TrueColor, &xvi) ||
		...
		_x.depth = 24;
		   ...
		   XMatchVisualInfo(_x.display, xrootid, 16, DirectColor,
				    &xvi)) {
		_x.vis = xvi.visual;
		_x.depth = 16;
	} else if (XMatchVisualInfo(_x.display, xrootid, 15, TrueColor, &xvi) ||
		   XMatchVisualInfo(_x.display, xrootid, 15, DirectColor,
				    &xvi)) {
		_x.vis = xvi.visual;
		_x.depth = 15;
	} else if (XMatchVisualInfo(_x.display, xrootid, 8, PseudoColor,
				    &xvi) ||
		   XMatchVisualInfo(_x.display, xrootid, 8, StaticColor,
				    &xvi)) {
		if (_x.depth > 8) {
...
static void runxevent(XEvent *xev) {
	...
	case MotionNotify:
		...
		gfx_mousetrack(w->client, m.xy.x, m.xy.y,
			       (m.buttons | _x.kbuttons) << shift, m.msec);
		break;
	...
	case KeyPress:
		...
		if (k == XK_F11) {
			...
			_xmovewindow(w, w->fullscreen ? w->screenrect
						      : w->windowrect);
			return;
		}
...
static Memimage *xattach(Client *client, char *label, char *winsize) {
	char                *argv[2];
	...
	if (winsize && winsize[0]) {
		 ...
		 */
		char *screen_resources, *display_resources, *geom, *geomrestype,
		    *home, *file, *dpitype;
		...
		if (display_resources == nil) {
			...
			XrmCombineDatabase(
			    XrmGetStringDatabase(display_resources), &database,
			    False);
		}

		if (XrmGetResource(database, "Xft.dpi", "String", &dpitype,
				   &dpires) == True) {
			if (dpires.addr) {
	...
	attr.border_pixel = 0;
	w->drawable = XCreateWindow(_x.display,  /* display */
				    xrootwin,    /* parent */
				    x,           /* x */
				    y,           /* y */
				    Dx(r),       /* width */
	...
	if (_x.clipboard == 0) {
		...
		_x.compoundtext =
		    XInternAtom(_x.display, "COMPOUND_TEXT", False);
		_x.takefocus = XInternAtom(_x.display, "WM_TAKE_FOCUS", False);
		_x.losefocus =
		    XInternAtom(_x.display, "_9WM_LOSE_FOCUS", False);
	...
	w->screenr = r;
	w->screenpm =
	    XCreatePixmap(_x.display, w->drawable, Dx(r), Dy(r), _x.depth);
...
Memimage *rpc_attach(Client *client, char *label, char *winsize) {
	Memimage *m;
...
void rpc_setlabel(Client *client, char *label) {
 ...
 */
static XGC xgc(XDrawable d, int fillstyle, int foreground) {
	XGC       gc;
	XGCValues v;
 ...
 */
static int setupcmap(XWindow w) {
	char   buf[30];
	int    i;
	u32int p, pp;
	XColor c;
	...
	if (_x.depth >= 24) {
		if (_x.usetable == 0) {
			_x.cmap =
			    XCreateColormap(_x.display, w, _x.vis, AllocNone);
		}
		...
		if (_x.usetable == 0) {
			_x.cmap =
			    XCreateColormap(_x.display, w, _x.vis, AllocAll);
			...
			for (i = 0; i < 128; i++) {
				c = _x.map7[i];
				if (!XAllocColor(_x.display, _x.cmap, &c)) {
					werrstr("can't allocate colors in "
						"7-bit map");
					return -1;
				}
...
void rpc_flush(Client *client, Rectangle r) {
	Xwin *w = (Xwin *)client->view;
	...
	XCopyArea(_x.display, w->screenpm, w->drawable, _x.gccopy, r.min.x,
		  r.min.y, Dx(r), Dy(r), r.min.x, r.min.y);
	XFlush(_x.display);
	xunlock();
}
...
static int _xconfigure(Xwin *w, XEvent *e) {
	...
	if (!w->fullscreen) {
		int     rx, ry;
		XWindow xw;
		if (XTranslateCoordinates(_x.display, w->drawable,
					  DefaultRootWindow(_x.display), 0, 0,
					  &rx, &ry, &xw)) {
			w->windowrect =
			    Rect(rx, ry, rx + xe->width, ry + xe->height);
		}
	}
	...
	if (w->screenpm != w->nextscreenpm) {
		XCopyArea(_x.display, w->screenpm, w->drawable, _x.gccopy,
			  r.min.x, r.min.y, Dx(r), Dy(r), r.min.x, r.min.y);
		XSync(_x.display, False);
	}
	w->newscreenr = r;
	return 1;
...
void rpc_resizeimg(Client *client) {
	xlock();
	_xreplacescreenimage(client);
	xunlock();
}

void rpc_gfxdrawlock(void) { xlock(); }

void rpc_gfxdrawunlock(void) { xunlock(); }

void rpc_topwin(Client *client) {
	Xwin *w = (Xwin *)client->view;
...
void rpc_resizewindow(Client *client, Rectangle r) {
	Xwin          *w = (Xwin *)client->view;
	XWindowChanges e;
	int            value_mask;
...
static int _xtoplan9mouse(Xwin *w, XEvent *e, Mouse *m) {
	...
	if (_x.putsnarf != _x.assertsnarf) {
		...
		XSetSelectionOwner(_x.display, XA_PRIMARY, w->drawable,
				   CurrentTime);
		if (_x.clipboard != None) {
			XSetSelectionOwner(_x.display, _x.clipboard,
					   w->drawable, CurrentTime);
		}
		XFlush(_x.display);
	}
	...
	case ButtonPress:
		 ...
		 * get to us.
		 */
		if (be->send_event && (~be->state & 0xFFFF) == 0 &&
		    (~be->button & 0xFF) == 0) {
			return -1;
		}
...
void rpc_setmouse(Client *client, Point p) {
	Xwin *w = (Xwin *)client->view;
...
static void xcursorarrow(Xwin *w) {
	if (_x.cursor != 0) {
...
void rpc_setcursor(Client *client, Cursor *c, Cursor2 *c2) {
	Xwin   *w = (Xwin *)client->view;
	XColor  fg, bg;
	XCursor xc;
	Pixmap  xsrc, xmask;
	...
	bg = _x.map[255];
	xsrc =
	    XCreateBitmapFromData(_x.display, w->drawable, (char *)src, 16, 16);
	xmask = XCreateBitmapFromData(_x.display, w->drawable, (char *)mask, 16,
				      16);
	xc = XCreatePixmapCursor(_x.display, xsrc, xmask, &fg, &bg,
...
static uchar *_xgetsnarffrom(Xwin *w, XWindow xw, Atom clipboard, Atom target,
			     int timeout0, int timeout) {
	Atom   prop, type;
	 ...
	 */
	prop = 1;
	XChangeProperty(_x.display, w->drawable, prop, target, 8,
			PropModeReplace, (uchar *)"", 0);
	XConvertSelection(_x.display, clipboard, target, prop, w->drawable,
			  CurrentTime);
	XFlush(_x.display);
	lastlen = 0;
	...
	XGetWindowProperty(_x.display, w->drawable, prop, 0,
			   ...
			   &fmt, &len, &dummy, &xdata);
	if ((type != target && type != XA_STRING && type != _x.utf8string) ||
	    len == 0) {
		if (xdata) {
			XFree(xdata);
		}
...
void __xputsnarf(char *data) {
	...
	e.state = ~0;
	e.button = ~0;
	XSendEvent(_x.display, w->drawable, True, ButtonPressMask,
		   (XEvent *)&e);
	XFlush(_x.display);
	xunlock();
	qunlock(&clip.lk);
...
static int _xselect(XEvent *e) {
	...
	if (xe->target == _x.targets) {
		...
		a[2] = _x.text;
		a[3] = _x.compoundtext;
		XChangeProperty(_x.display, xe->requestor, xe->property,
				XA_ATOM, 32, PropModeReplace, (uchar *)a,
				nelem(a));
	} else if (xe->target == XA_STRING || xe->target == _x.utf8string ||
		   xe->target == _x.text || xe->target == _x.compoundtext ||
		   ((name = XGetAtomName(_x.display, xe->target)) &&
		    strcmp(name, "text/plain;charset=UTF-8") == 0)) {
		/* text/plain;charset=UTF-8 seems nonstandard but is used by
		 * Synergy */
		/* if the target is STRING we're supposed to reply with Latin1
		 * XXX */
		qlock(&clip.lk);
		XChangeProperty(_x.display, xe->requestor, xe->property,
				xe->target, 8, PropModeReplace,
				(uchar *)clip.buf, strlen(clip.buf));
		qunlock(&clip.lk);
	} else {
...
char *_applegetsnarf(void) {
	...
	for (i = 1; i <= nitem; i++) {
		...
		for (j = 0; j < nflavor; j++) {
			...
			if (!UTTypeConformsTo(
				type, CFSTR("public.utf16-plain-text"))) {
				continue;
			}
 ...
 */
void rpc_bouncemouse(Client *c, Mouse m) {
	Xwin        *w = (Xwin *)c->view;
	XButtonEvent e;
	XWindow      dw;
	...
	XTranslateCoordinates(_x.display, w->drawable,
			      DefaultRootWindow(_x.display), m.xy.x, m.xy.y,
			      &e.x_root, &e.y_root, &dw);
