...
static void addrect(Rectangle *rp, Rectangle r) {
...
XImage *_xgetxdata(Memimage *m, Rectangle r) {
	int    x, y;
	uchar *p;
	...
	if (_x.usetable && m->chan == CMAP8) {
		for (y = r.min.y; y < r.max.y; y++) {
			for (x = r.min.x, p = byteaddr(m, Pt(x, y));
			     x < r.max.x; x++, p++) {
				*p = _x.toplan9[*p];
			}
		}
...
void _xputxdata(Memimage *m, Rectangle r) {
	int     x, y;
	uchar  *p;
	Point   tp, xdelta, delta;
	Xmem   *xm;
...
void _xdirtyxdata(Memimage *m, Rectangle r) {
	Xmem *xm;
