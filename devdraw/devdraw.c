...
void draw_initdisplaymemimage(Client *c, Memimage *m) {
	c->screenimage = m;
	m->screenref = 1;
	c->slot = 0;
	c->clientid = 1;
...
void gfx_replacescreenimage(Client *c, Memimage *m) {
	/*
...
static void drawrefresh(Memimage *m, Rectangle r, void *v) {
	Refx    *x;
	DImage  *d;
	Client  *c;
	Refresh *ref;
...
static void addflush(Client *c, Rectangle r) {
	int       abb, ar, anbb;
	Rectangle nbb, fr;
	...
	if (fr.min.x < fr.max.x) {
		// Unlock drawlk because rpc_flush may want to run on gfx
		// thread, and gfx thread might be blocked on drawlk trying to
		// install a new screen during a resize.
		rpc_gfxdrawunlock();
		qunlock(&drawlk);
...
static void dstflush(Client *c, int dstid, Memimage *dst, Rectangle r) {
	Memlayer *l;
...
static void drawflush(Client *c) {
	Rectangle r;
...
static int drawcmp(char *a, char *b, int n) {
	if (strlen(a) != n) {
		return 1;
	}
	return memcmp(a, b, n);
...
static DName *drawlookupname(Client *client, int n, char *str) {
	DName *name, *ename;
...
static int drawgoodname(Client *client, DImage *d) {
	DName *n;
...
static DImage *drawlookup(Client *client, int id, int checkname) {
	DImage *d;
...
static DScreen *drawlookupdscreen(Client *c, int id) {
	DScreen *s;
...
static DScreen *drawlookupscreen(Client *client, int id, CScreen **cs) {
	CScreen *s;
...
static Memimage *drawinstall(Client *client, int id, Memimage *i,
			     DScreen *dscreen) {
	DImage *d;
...
static Memscreen *drawinstallscreen(Client *client, DScreen *d, int id,
				    DImage *dimage, DImage *dfill, int public) {
	Memscreen *s;
	CScreen   *c;
...
static void drawdelname(Client *client, DName *name) {
	int i;
...
static void drawfreedscreen(Client *client, DScreen *this) {
	DScreen *ds, *next;
...
static void drawfreedimage(Client *client, DImage *dimage) {
	int       i;
	Memimage *l;
	DScreen  *ds;
...
static void drawuninstallscreen(Client *client, CScreen *this) {
	CScreen *cs, *next;
...
static int drawaddname(Client *client, DImage *di, int n, char *str,
		       char **err) {
...
static int drawclientop(Client *cl) {
	int op;
...
static Memimage *drawimage(Client *client, uchar *a) {
	DImage *d;
...
static void drawrectangle(Rectangle *r, uchar *a) {
...
static void drawpoint(Point *p, uchar *a) {
	p->x = BGLONG(a + 0 * 4);
	p->y = BGLONG(a + 1 * 4);
}

static Point drawchar(Memimage *dst, Point p, Memimage *src, Point *sp,
		      DImage *font, int index, int op) {
	FChar    *fc;
	Rectangle r;
	Point     sp1;
...
static uchar *drawcoord(uchar *p, uchar *maxp, int oldx, int *newx) {
	int b, x;
...
int draw_datawrite(Client *client, void *v, int n) {
	...
	while ((n -= m) > 0) {
		...
		case 'A':
			...
			if (drawinstallscreen(client, 0, dstid, ddst, dsrc,
					      a[13]) == 0) {
				goto Edrawmem;
			}
			continue;
		...
		/* ellipse: 'e' dstid[4] srcid[4] center[2*4] a[4] b[4] thick[4]
		 * sp[2*4] alpha[4] phi[4]*/
		case 'e':
		case 'E':
			...
			dstflush(client, dstid, dst,
				 Rect(p.x - e0 - j, p.y - e1 - j,
				      p.x + e0 + j + 1, p.y + e1 + j + 1));
			continue;
		...
		/* draw line: 'L' dstid[4] p0[2*4] p1[2*4] end0[4] end1[4]
		 * radius[4] srcid[4] sp[2*4] */
		case 'L':
		...
		case 'N':
			...
			if (c) {
				if (drawaddname(client, di, j, (char *)a + 7,
						&err) < 0) {
					goto error;
				} else {
		...
		/* filled polygon: 'P' dstid[4] n[2] wind[4] ignore[2*4]
		 * srcid[4] sp[2*4] p0[2*4] dp[2*2*n] */
		/* polygon: 'p' dstid[4] n[2] end0[4] end1[4] radius[4] srcid[4]
		 * sp[2*4] p0[2*4] dp[2*2*n] */
		case 'p':
		case 'P':
			m = 1 + 4 + 2 + 4 + 4 + 4 + 4 + 2 * 4;
			if (n < m) {
			...
			doflush = 0;
			if (dstid == 0 ||
			    (dst->layer && dst->layer->screen->image->data ==
					       client->screenimage->data)) {
		...
		/* string: 's' dstid[4] srcid[4] fontid[4] P[2*4] clipr[4*4]
		 * sp[2*4] ni[2] ni*(index[2]) */
		/* stringbg: 'x' dstid[4] srcid[4] fontid[4] P[2*4] clipr[4*4]
		 * sp[2*4] ni[2] bgid[4] bgpt[2*4] ni*(index[2]) */
		case 's':
		case 'x':
			...
			p.y -= font->ascent;
			dstflush(client, dstid, dst,
				 Rect(p.x, p.y, q.x, p.y + Dy(font->image->r)));
			continue;
		...
		case 'S':
			...
			if (dscrn == 0 ||
			    (dscrn->public == 0 && dscrn->owner != client)) {
				goto Enodrawscreen;
			}
		...
		case 't':
			...
			if (lp[0]->layer->screen->image->data ==
			    client->screenimage->data) {
				for (j = 0; j < nw; j++) {
