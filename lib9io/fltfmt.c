...
enum { FDIGIT = 30, FDEFLT = 6, NSIGNIF = 17 };
...
static double pows10[] = {
    ...
    1e154, 1e155, 1e156, 1e157, 1e158, 1e159,
};
	...
	for (b = a + n - 1; b >= a; b--) {
		...
		if (c >= '0') {
			if (c == '0' && b == a) {
				 ...
				 * have gotten this far.  (e.g., 10000-1 =>
				 * 09999)
				 */
				*b = '9';
				return 1;
 ...
 */
static void xfmtexp(char *p, int e, int ucase) {
	char se[9];
	int  i;
 ...
 */
static void xdtoa(double f, char *s, int *exp, int *neg, int *ns) {
...
int __efgfmt(Fmt *fmt) {
	...
	case 'e':
		...
		if (1 + prec >= ndigits) {
			...
			assert(realchr != 'g');
			newndigits = 1 + prec;
			if (digits[newndigits] >= '5' &&
			    xadd1(digits, newndigits)) {
				/*
