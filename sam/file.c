...
static void wrdelete(Buffer *delta, int seq, int mod, uint p0, uint p1) {
	Undo u;
...
void flushmerge(void) {
	...
	if (merge.n != 0) {
		wrdelete(&f->epsilon, f->seq, true, merge.p0,
			 merge.p0 + merge.n);
	}
	if (merge.nbuf != 0) {
		wrinsert(&f->epsilon, f->seq, true, merge.p0 + merge.n,
			 merge.buf, merge.nbuf);
	}
	merge.f = NULL;
	merge.n = 0;
	merge.nbuf = 0;
...
void mergeextend(File *f, uint p0) {
	uint mp0n;

	mp0n = merge.p0 + merge.n;
	if (mp0n != p0) {
		bufread(&f->buf, mp0n, merge.buf + merge.nbuf, p0 - mp0n);
		merge.nbuf += p0 - mp0n;
		merge.n = p0 - merge.p0;
	}
}
 ...
 */
void loginsert(File *f, uint p0, Rune *s, uint ns) {
	if (f->rescuing) {
		return;
	}
	if (ns == 0) {
	...
	if (merge.f != f || p0 - (merge.p0 + merge.n) > Maxmerge /* too far */
	    || merge.nbuf + ((p0 + ns) - (merge.p0 + merge.n)) >=
		   RBUFSIZE) { /* too long */
		flushmerge();
	}

	if (ns >= RBUFSIZE) {
		if (!(merge.n == 0 && merge.nbuf == 0 && merge.f == NULL)) {
...
void logdelete(File *f, uint p0, uint p1) {
	if (f->rescuing) {
		return;
	}
	if (p0 == p1) {
	...
	if (merge.f != f || p0 - (merge.p0 + merge.n) > Maxmerge /* too far */
	    || merge.nbuf + (p0 - (merge.p0 + merge.n)) >=
		   RBUFSIZE) { /* too long */
		flushmerge();
		merge.f = f;
		merge.p0 = p0;
...
void filesetname(File *f, String *s) {
	if (!f->unread) { /* This is setting initial file name */
...
void fileunsetdot(File *f, Buffer *delta, Range dot) {
	Undo u;
...
void fileunsetmark(File *f, Buffer *delta, Range mark) {
	Undo u;
...
uint fileload(File *f, uint p0, int fd, bool *nulls) {
	if (f->seq > 0) {
		panic("undo in file.load unimplemented");
	}
	return bufload(&f->buf, p0, fd, nulls);
}

int fileupdate(File *f, bool notrans, bool toterm) {
	uint p1, p2;
	int  mod;
...
long undoseq(File *f, int isundo) {
	if (isundo) {
		return f->seq;
	}
